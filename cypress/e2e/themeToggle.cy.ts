...
describe("Theme toggle", () => {
  ...
  it("should be visible the toggle and the toggle element", () => {
    cy.get("[data-test=toggle]").should("be.visible");
    cy.get("[data-test=toggle-element]").should("be.visible");
  });

  it("should the toggle element moves by clicking the toggle", () => {
    cy.get("[data-test=toggle]").as("toggle");
    cy.get("[data-test=toggle-element]").as("toggle-element");
  ...
  it("should the theme changes by clicking the toggle", () => {
    cy.get("[data-test=logo] > h1").as("logo-text");
    cy.get("body").as("body");

    cy.get("@logo-text").should("have.css", "color", "rgb(0, 0, 0)");
    cy.get("@body").should("have.css", "background-color", "rgb(255, 255, 255)");

    cy.get("[data-test=toggle]").first().click();
